...
> 的 mTLS 连接与 `Control` 流（握手 + 心跳）；通用的请求/响应分发信封及三个 REST 接口族
> —— **Docker 镜像**（`GET`/`POST`/`DELETE /api/v1/agents/<id>/images`：列举、SSE 流式
> 拉取、删除）、**Docker 容器**（`/api/v1/agents/<id>/containers`：列举、docker run 语义的
> 创建并启动、inspect、start/stop/restart、删除，以及支持 `?follow` 的 SSE **日志流**）与
> agent **工作目录文件系统**（`GET`/`POST`/`PUT`/`DELETE
...
> 注册表；以及经 `ControllerPeer` 平面的跨 pod API 中继（多副本 HA），含 follow 日志流的
> 取消传播。仍**处于设计、尚未实现**：Docker **事件**上报、controller 侧的日志中继入
> Loki、端口转发，以及跨 pod 的端口转发数据面 ——
> 参见 [路线图](#路线图)。请勿将其作为承载真实业务的控制面部署。
>
> 当前以 **v0.x pre-release** 形式发布：REST API、通信协议与配置在 v1.0.0
> 之前可能不作兼容性保证地变更。
...
`hostlink` 由单个 Go module 构建出两个二进制 —— **均命名为 `hostlink`**，由运行环境天然区分：

- **controller** —— 以容器形式（镜像 `hostlink`）运行在云端、Kubernetes 之内。它是控制面，是 gRPC **服务端**，为 agent 终结双向 TLS，并（按设计）聚合指标、分配公网端口、在副本间路由被转发的 TCP。
- **agent** —— 以 systemd 服务的形式运行在每台外部主机上（二进制 `/usr/local/bin/hostlink`）。它是 gRPC **客户端**：主动拨出连接到 controller（因此可在 NAT 后工作、无需任何入站防火墙规则），驱动本地 Docker 守护进程，上报指标，承载隧道。

> **命名说明。** 原 `hostlink-controller` 名称已弃用：controller 只会以容器形式跑在 Kubernetes 里，agent 是主机上唯一安装的 hostlink 组件，因此在各自上下文中直接叫 `hostlink` 没有歧义。文档以角色名 “controller”/“agent” 行文；两个二进制的 `version.Name` 均为 `hostlink`，配置文件均为 `/etc/humble-mun/hostlink.yaml`。
...
外部主机（NAT 之后）                       云端（Kubernetes）
  hostlink（agent）                  hostlink（controller，≥2 副本）
       ...
       │     Heartbeat（刷新 TTL） ─────────────► │
       │                                          │
       │ ◄──────────── Command（推送） ─────────── │  AgentRequest（images/containers/fs/…）/ OpenForward / ExposeRule
       │                                          │
...
> **当前已实现的部分：** mTLS 拨号、`Control` 流与 `Hello` / `Heartbeat` 交互；通用的 `AgentRequest`/`AgentResult` 分发信封（含 `AgentProgress` / `final` 流式扩展）及 Docker 镜像 REST 接口（`GET` 列举 `images.list`、`POST` 经 SSE 流式拉取 `images.pull`、`DELETE` 删除 `images.remove`，位于 `/api/v1/agents/<id>/images`）；Docker **容器**接口（`/api/v1/agents/<id>/containers`：列举、docker run 语义的创建并启动、inspect、start/stop/restart、删除，以及 SSE **日志流** `.../logs`，支持 `?follow`，客户端断开经 `request.cancel` 取消 agent 侧的流）；agent **工作目录文件系统**接口（`GET`/`POST`/`PUT`/`DELETE /api/v1/agents/<id>/files`）；**指标聚合**端点（`GET /api/v1/metrics`，反向拉取、打标并合并每个 agent 配置的 exporter）；Redis 的 `agent→pod` 注册表；以及经 `ControllerPeer.Dispatch`（一元）、`ControllerPeer.DispatchStream`（流式）与 `ControllerPeer.Upload`（客户端流式，用于文件系统写入）的跨 pod API 请求中继。`Forward`、Docker **事件**上报，以及跨 pod 的端口转发数据面仍为占位桩或尚未实现。参见[路线图](#路线图)。
...
连接建立后，agent 开启 `Control(stream AgentEvent) returns (stream Command)`。controller 沿 `Command` 流推送命令；agent 沿 `AgentEvent` 流上报其握手、心跳与 Docker 事件。

controller→agent 的绝大部分工作走通用的 **`AgentRequest`** 信封：`method` 命名操作（`images.*`、`containers.*`、`fs.*`、`metrics.scrape`），`payload` 为方法专属的 JSON，响应（`AgentResult`，流式方法另有 `AgentProgress` 帧）按 `request_id` 关联。fire-and-forget 的 `request.cancel` 元方法用于取消进行中的流式请求 —— follow 日志流的 HTTP 客户端断开后正是靠它终止 agent 侧的流。容器生命周期就实现在这条通道上（`containers.*` 方法）；proto 里的 `DockerOp` 命令是未使用的历史占位。`OpenForward` 与 `ExposeRule`（端口转发）仍处于设计阶段。
...
`GET /api/v1/agents/<id>/containers/<containerId>/logs` 以 **SSE** 流式返回容器日志：每行日志一个 `data:` 事件（`{"stream":"stdout"|"stderr","line":"..."}`），以 `{"done":true,...}` 收尾。查询参数与 `docker logs` 对齐：`follow` 保持流打开以接收新输出，`tail` 限定初始回溯行数，`since` 限定起始时间，`timestamps` 为每行加时间前缀。agent 先 inspect 容器的 TTY 模式以选择正确的线上格式（裸流 vs `stdcopy` 复用流），解复用出 stdout/stderr 后按行成帧，经**无损**流类别回传（不丢行、有背压）。

follow 流是无限的，因此终止是显式的：HTTP 客户端断开（或请求被放弃）后，controller 沿该 agent 的 `Control` 流发送 fire-and-forget 的 `request.cancel`，agent 随即取消底层的 `docker logs` follow。该机制可穿透跨 pod 场景 —— 取消 `ControllerPeer.DispatchStream` 中继会让持有 pod 代为发出取消。

> **日志采集入 Loki（已决策，尚未实现）。** agent 主机位于**受限网络，唯一被放行的路径是 agent→controller 的 mTLS 连接**，因此逐主机部署采集器（Alloy/promtail 直推 Loki）不可行。既定方案是经 controller 中继（“方案 B”）：controller 侧的 log-relay 组件对本副本持有的每个 agent 的目标容器发起 `containers.logs` follow，为每行打上 `{agent_id, container_name}` 标签，推送到集群内 Alloy 的 `loki.source.api` 端点（批量/重试/relabel 交由 Alloy 完成）。待解决的问题：重连后基于 `since` 的断点续采、agent 漂移到其他副本时 relay 的所有权跟随，以及共享 `Control` 流上的带宽预算。参见[路线图](#路线图)。
...
服务定义于 `pkg/api/hostlink/v1/`。`AgentLink`（agent↔controller）有两个双向流 RPC；`ControllerPeer`（controller↔controller）有三个中继 RPC（一元、服务端流式与客户端流式）：
...
| `GET /api/v1/agents/<agentId>/containers` | 列出该 agent 上的容器（`containers.list`）。`?all=true` 包含已停止的容器（docker ps -a）。 |
| `POST /api/v1/agents/<agentId>/containers` | 创建**并启动**容器（docker run 语义，`containers.create`）。JSON 请求体：`image`（必填，须已拉取到主机），可选 `name`、`cmd`、`entrypoint`、`env`（`KEY=value` 字符串）、`workingDir`、`labels`、`ports`（`{containerPort, protocol?, hostIp?, hostPort?}`）、`binds`（`/host:/container[:opts]`）、`networkMode` / `pidMode`（如 `"host"`）、`restartPolicy`（配 `maxRetryCount`）、`autoRemove`。成功返回 `201` 与 `{"id":"...","warnings":[...]}`。 |
| `GET /api/v1/agents/<agentId>/containers/<containerId>` | Inspect 容器（`containers.inspect`）：身份、状态与生效的运行配置。容器不存在返回 `404`。 |
| `GET /api/v1/agents/<agentId>/containers/<containerId>/logs` | 以 **SSE** 流式返回容器日志（`containers.logs`）：每行一个事件 `{"stream","line"}`，以 `{"done":true,...}` 收尾。选项与 docker logs 对齐：`?follow=true`、`?tail=100`、`?since=<RFC3339|unix>`、`?timestamps=true`。客户端断开会（跨 pod）经 `request.cancel` 取消 agent 侧的流。 |
| `POST /api/v1/agents/<agentId>/containers/<containerId>/start` | 启动已停止的容器（`containers.start`）。成功返回 `204`。 |
| `POST /api/v1/agents/<agentId>/containers/<containerId>/stop` | 停止运行中的容器（`containers.stop`）。可选 `?timeout=<秒>` 指定 daemon 强杀前的宽限期。成功返回 `204`。 |
| `POST /api/v1/agents/<agentId>/containers/<containerId>/restart` | 重启容器（`containers.restart`），`?timeout` 同 stop。成功返回 `204`。 |
| `DELETE /api/v1/agents/<agentId>/containers/<containerId>` | 删除容器（`containers.remove`）。`?force=true` 删除运行中的容器（docker rm -f），`?volumes=true` 一并删除其匿名卷。未加 `force` 删除运行中的容器返回 `409`。 |
...
> 两个二进制均命名为 `hostlink`（本地构建以不同输出目录区分）；生产环境中 controller 以 `hostlink` 容器镜像交付（`make build`），agent 以 `hostlink` 主机二进制交付（`make agent`）。
...
所有参数也可通过环境变量设置：将参数名大写、把 `-` 替换为 `_`，并加上 `HM_` 前缀（例如 `--controller-endpoint` → `HM_CONTROLLER_ENDPOINT`）。配置也可以 YAML 文件形式置于 `/etc/humble-mun/hostlink.yaml`（文件名取自二进制的 `version.Name`，controller 与 agent 均为 `hostlink`），以参数名为键；该文件会被监听并在运行时重新加载。优先级：命令行参数 > 环境变量 > 配置文件。

### Agent 参数

| 参数 | 默认值 | 说明 |
|------|---------|-------------|
| `--controller-endpoint` | —— | 要拨号的 controller gRPC endpoint 地址，形如 `host:port`（必填） |
...
| `--data-dir` | —— | `files` 接口服务的工作目录（浏览/下载/上传/删除）。留空则禁用 `files` 接口 |

> `scrape-targets`（agent 的上游 exporter 列表，供指标聚合拉取）是结构化 YAML 列表，仅能经配置文件设置 —— 其形态见 `deploy/hostlink.yaml` 中的注释示例。
...
| `--agent-scrape-timeout` | `5s` | `GET /api/v1/metrics` 聚合的逐 agent 超时；保持小于 Prometheus 的 `scrape_timeout` |
| `--grpc-max-recv-msg-size` | 16 MiB | 从 agent 或兄弟副本接收的单条 gRPC 消息上限（针对大体积一元结果；流式方法分块传输、不受其影响） |
...
- **行为：** 经 mTLS 拨出连接到 controller 的公网 gRPC endpoint，运行 `Control` 流（`Hello` + 周期性 `Heartbeat`），并以**进程内重连**（指数退避 + 抖动，配合 HTTP/2 keepalive）扛过 controller 重启/发版而无需进程退出。它经惰性 `client.FromEnv` Docker 客户端服务 controller 推送的 `AgentRequest`：Docker **镜像**方法（`images.list` / `images.pull` 流式 / `images.remove`）、**容器**方法（`containers.list` / `containers.create`（run）/ `containers.inspect` / `containers.start` / `containers.stop` / `containers.restart` / `containers.remove` / `containers.logs` 流式）与**文件系统**方法（`fs.stat` / `fs.list` / `fs.read` 分块下载 / `fs.write` 分块上传 / `fs.mkdir` / `fs.remove`，作用于 `--data-dir` 工作目录，所有路径均解析在其内、拒绝越界）。Docker **事件**上报、承载隧道，以及监听 `127.0.0.1:9100` 的 node_exporter sidecar 仍为计划项（参见[路线图](#路线图)）。由于 Docker 客户端是惰性的，单元文件把 `docker.service` 保留为软（`Wants`）顺序依赖，而非硬性要求。
...
- [x] 容器 CRUD 与生命周期（`/api/v1/agents/<id>/containers`）：`containers.list` / `containers.create`（docker run 语义，支持端口映射、binds、`--net`/`--pid` host 模式、重启策略）/ `containers.inspect` / `containers.start` / `containers.stop` / `containers.restart` / `containers.remove`
- [x] 容器**日志流**（`.../containers/<id>/logs`，SSE，`?follow`/`?tail`/`?since`/`?timestamps`）：感知 TTY 的 stdout/stderr 解复用、按行成帧、无损传输；以及 `request.cancel` 元方法，被放弃的 follow 流会在 agent 侧被终止，且可跨 pod 传播

### 进行中 / 计划中（MVP）

- [ ] Docker **事件**上报（`DockerEvent`：容器 start/stop/die 驱动暴露生命周期）
- [ ] Controller 侧**日志中继入 Loki**（“方案 B” —— agent 仅能到达 controller，故由 relay 对所持 agent 逐容器 follow `containers.logs`，打 `{agent_id, container_name}` 标签后推送到集群内 Alloy 的 `loki.source.api`；需解决基于 `since` 的断点续采、重连漂移时的所有权跟随，以及 Control 流带宽预算）
