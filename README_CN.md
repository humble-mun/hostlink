...
| agent | 客户端 cert/key（`--tls-cert-path` / `--tls-key-path`） | controller CA bundle（`--tls-ca-path`） |

controller 要求且验证 agent 客户端证书（`RequireAndVerifyClientCert`）；agent 验证 controller 的服务名（`--tls-server-name`，默认取 endpoint 主机名）。**没有不安全回退** —— 若证书缺失或无效，连接将硬性失败。

---

## 命令行参数

所有参数也可通过环境变量设置：将参数名大写、把 `-` 替换为 `_`，并加上 `HM_` 前缀（例如 `--controller-endpoint` → `HM_CONTROLLER_ENDPOINT`）。配置也可以 YAML 文件形式置于 `/etc/humble-mun/<binary>.yaml`。
...
| `--tls-cert-path` | —— | agent 为 mTLS 向 controller 出示的客户端证书 |
| `--tls-key-path` | —— | 与客户端证书匹配的私钥 |
| `--tls-ca-path` | —— | 用于验证 controller 证书的 CA bundle |
| `--tls-server-name` | —— | 据以验证 controller 证书的服务名（默认取 endpoint 主机名） |
...
- **形态：** 一个 Kubernetes **Deployment，≥2 副本**（高可用）。
...
- **Ingress（L4 LoadBalancer）：** 供 agent 拨出的 mTLS gRPC 端口，外加（按设计）一段路由到每个副本、用于隧道暴露的**预留 TCP 端口段**（例如 `1025–2025`）。暴露一个端口是应用层的“从池中分配 + 写 Redis”；pod 已经监听整段端口，因此无需为每次暴露改动 Service/LB。
- **依赖：** Redis（注册表 + 端口分配）。副本之间经内部 gRPC 互相转发。
...
- **形态：** 一个以 **systemd 服务**运行的静态 Go 二进制。
- **行为：** 拨出连接到 controller 的公网 gRPC endpoint；管理本地 Docker 守护进程；承载隧道；与一个监听 `127.0.0.1:9100` 的 node_exporter sidecar 一同运行。
