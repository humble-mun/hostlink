...
> 的 mTLS 连接与 `Control` 流（握手 + 心跳）；通用的请求/响应分发信封及第一个
> REST 接口（`GET /api/v1/agents/<id>/images`，由 agent 端 Docker 客户端服务）；
> 基于 Redis 的 `agent→pod` 注册表；以及经 `ControllerPeer` 平面的跨 pod API
> 中继（多副本高可用）。仍**已完成设计但尚未实现**：容器*生命周期*操作
...
> **当前已实现的部分：** mTLS 拨号、`Control` 流与 `Hello` / `Heartbeat` 交互；通用的 `AgentRequest`/`AgentResult` 分发信封及 `GET /api/v1/agents/<id>/images`（agent 列出本地 Docker 镜像）；Redis 的 `agent→pod` 注册表；以及经 `ControllerPeer.Dispatch` 的跨 pod API 请求中继。`Forward`、`DockerOp` 执行、指标聚合，以及跨 pod 的端口转发数据面仍为占位桩或尚未实现。参见[路线图](#路线图)。
...
服务定义于 `pkg/api/hostlink/v1/`。`AgentLink`（agent↔controller）有两个双向流 RPC；`ControllerPeer`（controller↔controller）有一个一元 RPC：
...
| 消息 | 方向 | 用途 |
|---------|-----------|---------|
| `AgentEvent{Hello / Heartbeat / DockerEvent / AgentResult}` | agent → controller | 握手、TTL 刷新、容器上报、对 `AgentRequest` 的回复 |
| `Command{OpenForward / DockerOp / ExposeRule / AgentRequest}` | controller → agent | 开启转发流、执行 Docker 操作、增删暴露规则，或一条通用的按 method 分发的请求 |
| `AgentRequest{request_id, method, payload}` / `AgentResult{request_id, code, payload, error}` | 双向 | 通用 API 调用：`method` 命名操作（如 `images.list`），`payload` 为 JSON，`code` 对应 HTTP 状态，按 `request_id` 关联 |
| `DispatchRequest{agent_id, AgentRequest}` | controller → controller | 为 `ControllerPeer` 这一跳给 `AgentRequest` 包上路由键 |
...
| 方法与路径 | 说明 |
|---------------|-------------|
| `GET /api/v1/agents/<agentId>/images` | 列出指定 agent 上的 Docker 镜像。向该 agent 的 `Control` 流分发 `images.list`（本地处理，或经 `ControllerPeer` 中继到持有它的 pod），原样返回 agent 的 JSON。若该 agent 未连接到任何可达副本则返回 404。 |
...
| `--redis-url` | —— | 承载跨 pod `agent→pod` 注册表的 Redis URL；留空即内存态单副本模式 |
...
- **行为：** 经 mTLS 拨出连接到 controller 的公网 gRPC endpoint，运行 `Control` 流（`Hello` + `Heartbeat`），并服务 controller 推送的 `AgentRequest` —— 目前是 `images.list`：它打开一个 Docker 客户端（`client.FromEnv`，惰性 —— 直到有请求才真正连接守护进程）并以 JSON 返回本地镜像列表。更广的 Docker 生命周期（`DockerOp`/事件）、承载隧道，以及监听 `127.0.0.1:9100` 的 node_exporter sidecar 仍为计划项（参见[路线图](#路线图)）。由于 Docker 客户端是惰性的、且仅 `images.list` 使用它，单元文件把 `docker.service` 保留为软（`Wants`）顺序依赖，而非硬性要求。
...
- [x] 带 `Hello` 握手与周期性 `Heartbeat` 的 `Control` 流
- [x] 通用 `AgentRequest`/`AgentResult` 分发信封与关联；由 agent 端 Docker 客户端服务的 `GET /api/v1/agents/<id>/images`（`images.list`）
- [x] Redis `agent→pod` 注册表（写入/比较删除/TTL 刷新）与经 `ControllerPeer.Dispatch` 的跨 pod API 请求中继，含陈旧映射的拒绝并重试
