...
> REST 接口族（`GET /api/v1/agents/<id>/images` 列镜像、`POST` 经 SSE 流式拉取镜像、
> `DELETE` 删除镜像，均由 agent 端 Docker 客户端服务）；基于 Redis 的 `agent→pod`
...
| `AgentProgress{request_id, payload}` | agent → controller | 流式操作（如 `images.pull`）的中间进度帧，`payload` 为 JSON，按 `request_id` 关联，终结时以一条 `final=true` 的 `AgentResult` 收尾 |
...
- **行为：** 经 mTLS 拨出连接到 controller 的公网 gRPC endpoint，运行 `Control` 流（`Hello` + `Heartbeat`），并服务 controller 推送的 `AgentRequest` —— 目前是 Docker 镜像类方法（`images.list` 列举、`images.pull` 流式拉取并以 `AgentProgress` 逐帧上报进度、`images.remove` 批量删除）：它打开一个 Docker 客户端（`client.FromEnv`，惰性 —— 直到有请求才真正连接守护进程）。更广的 Docker 生命周期（`DockerOp`/事件）、承载隧道，以及监听 `127.0.0.1:9100` 的 node_exporter sidecar 仍为计划项（参见[路线图](#路线图)）。由于 Docker 客户端是惰性的、且仅镜像方法使用它，单元文件把 `docker.service` 保留为软（`Wants`）顺序依赖，而非硬性要求。
